...
mode = ""
while savefile == "data":
    savefile = input("Crawler name: ").lower()
savepath = os.getcwd() + "\\" + savefile + "\\"

if lookup("data.txt",savefile):
    print("A crawler with the same name already exists.\n")
    while mode != "O" and mode != "R":
        mode = input("Overwrite or resume (O/R)? ")
        if mode == "O":
            txtChangeLine("data.txt",numAtLine("data.txt",savefile)+1,1)
            new = True
else:
    ...
    new = True
if new:
    with open(savepath + savefile + ".txt","w+") as crawl:
        print("A fresh file, " + savefile + ".txt, was created successfully.\n")
        crawl.write(input("URL to start crawling (use http & ending slash): ") + "\n")

savelinenum = numAtLine("data.txt",savefile)
startline = int(txtAtLinenum("data.txt",savelinenum+1))
if mode == "R":
...
for i in range(iterations):
    ...
    #Initialize
    urlstart = txtAtLinenum(savepath + savefile + ".txt",startline+i)
    if not(urlstart):
        break
    try:
        print("\n-- Access #%s --" % (i+1))
        ...
        txtChangeLine("data.txt",savelinenum+1,startline+i+1)
        continue
    ...
    for num, tag in enumerate(atags):
        ...
        if not(re.search('''\.je?pg''',link.lower())) and not(lookup(savepath + savefile + ".txt",link)):
            
            print("Entered check ...")#
            
            try:
                print("Writing link ...")#
                with open(savepath + savefile + ".txt","a+") as crawl: ### This might be slower than using an always-open crawl file
                    crawl.write(link + "\n")
    ...
    for n,img in enumerate(imglist):
        ...
        try:
            ...
            continue
        if imgsize[0] > 400 and imgsize[1] > 400:                                       #Exclude thumbnail sizes or other low-res images
            time.sleep(0.01)
